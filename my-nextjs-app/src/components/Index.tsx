...
const Index = ({ setPrediction, setProb }) => {
  const [selectedFile, setSelectedFile] = useState(null);
  const [prediction, setPredictionState] = useState(""); // Define the prediction state here
  const [imagePath, setImagePath] = useState("");
  const [loading, setLoading] = useState(false);
  const [imgDimensions, setImgDimensions] = useState({ width: 0, height: 0 });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(response);
      setPredictionState(String(response.data.prediction)); // Set the prediction here
      setProb(response.data.prediction_probability);
      setImagePath(response.data.image_path);
    } catch (error) {
      console.error("Error:", error);
    ...
    <div className="max-w-3xl mx-left p-6 space-y-6 mt-20">
      ...
      >
        ...
        <button
          type="submit"
          disabled={loading}
          aria-label="Upload and predict chicken health"
          className={`bg-primary-blue text-white rounded-full py-3 px-6 font-semibold transition duration-300 ease-in-out transform ${
            loading ? "opacity-50 cursor-not-allowed" : "hover:bg-blue-700"
          } w-auto text-left`}
        >
